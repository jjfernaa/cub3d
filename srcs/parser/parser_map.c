...
// Procesa una línea durante el conteo (extrae texturas/colores)
static int	process_count_line(char *line, t_game *game, int *fd)
{
...
// Valida una línea del mapa durante la lectura
...
// Procesa una línea del mapa (guarda y valida)
static int	process_map_line(char *line, t_game *game, int *i, int *fd)
{
    int	len;
...
// Maneja líneas no pertenecientes al mapa
static int	handle_non_map_line(char *line, int map_started, int *fd)
{
...
int	count_lines(char *file, t_game **game)
{
    ...
    while (line)
    {
        if (process_count_line(line, *game, &fd) != 0)
            return (1);
        free(line);
        line = get_next_line(fd);
    }
